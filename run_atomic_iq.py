...
7. postprocess_portfolio_diagnostics.py
   Convenience post-processing step that reads OOS_results/result.pickle
   and writes portfolio_diagnostics.csv with implementation diagnostics
   such as effective number of assets, maximum weight, turnover, and
...
def main():
    for script in [
        ...
        "aiq_sr_tests.py",
        "postprocess_portfolio_diagnostics.py",
    ]:
        run_step(script)
